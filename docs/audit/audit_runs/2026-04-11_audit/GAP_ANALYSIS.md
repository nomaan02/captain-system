...
| G-ONL-004 | B1-Features | b1_features.py:863-864 | Doc 33 PG-21 §4 | `[RESOLVED]` | HIGH | `_get_overnight_range` implemented using prior close and current open from TopstepX daily bars |
| G-ONL-005 | B1-Features | b1_features.py:965-972 | Doc 33 PG-21 §4 | `[RESOLVED]` | HIGH | Options stubs documented as DATA_UNAVAILABLE (TopstepX futures-only); one-time session log; AIM-02/03 degrade to neutral modifier |
| G-ONL-006 | B1-Features | b1_features.py:938-940 | Doc 33 PG-21 §4 | `[RESOLVED]` | HIGH | `_get_trailing_pcr` documented as DATA_UNAVAILABLE (requires options volume); AIM-02 degrades to skew_z only at 0.4 weight |
...
| G-ONL-013 | B3 | aim_compute.py:175-178 | Doc 33 PG-23 §3 | `[RESOLVED]` | HIGH | `run_aim_aggregation()` now loads session_budget_weights from D26 and includes in return dict |
...
| G-ONL-017 | B4 | b4_kelly_sizing.py:132-141 | Doc 33 PG-24 L4 | `[RESOLVED]` | CRITICAL | L4 robust formula replaced with spec-correct `f_robust = mu/(mu²+var) if mu>0 else 0` inline computation; b1_features delegation removed |
| G-ONL-018 | B4 | b4_kelly_sizing.py:252-260 | Doc 33 PG-24 L6→L7 | `[RESOLVED]` | HIGH | Sizing override moved to pre-TSM position (Step 5b on Kelly fraction between L6 ceiling and L7 per-account sizing) |
| G-ONL-019 | B4 | b4_kelly_sizing.py:190-193 | Doc 33 PG-24 L7 | `[RESOLVED]` | HIGH | `risk_per_contract` now uses spec formula `strategy_sl * point_value + expected_fee` as primary; EWMA avg_loss removed |
...
| G-ONL-021 | B5B | b5b_quality_gate.py:49-77 | Doc 33 PG-25B | `[RESOLVED]` | HIGH | Quality gate now computes `dollar_per_contract = quality_score / total_contracts` with floor/ceiling check per spec |
...
| G-ONL-024 | B5C | b5c_circuit_breaker.py:296-325 | Doc 33 PG-27B L2 | `[RESOLVED]` | HIGH | L2 now implements dollar-budget check: `remaining = E - |L_t|; IF remaining < rho_j → BLOCK`. Trade-count ceiling replaced with spec-compliant dollar budget |
| G-ONL-025 | B5C | b5c_circuit_breaker.py:375-437 | Doc 33 PG-27B L4 | `[RESOLVED]` | HIGH | L4 now uses `rolling_basket_sharpe(lookback=60d)` from D03 trade history. Analytical formula replaced with empirical per-trade Sharpe |
...
| G-ONL-028 | B6 | b6_signal_output.py:94-134 | Doc 20 PG-26 | `[RESOLVED]` | CRITICAL | Command B1 now calls `sanitise_for_gui()` before GUI WebSocket push; strips all 9 PROHIBITED_EXTERNAL_FIELDS. Redis channel retains full blob for internal use |
| G-ONL-029 | B6 | b6_signal_output.py:94-134 | Doc 20 PG-26 | `[RESOLVED]` | HIGH | Signal blob reduced to 6 spec fields at top level (asset, direction, size, tp_level, sl_level, timestamp) plus routing keys; internal context moved to `_context` sub-dict |
| G-ONL-030 | B6 | b6_signal_output.py (whole) | Doc 20 PG-26 | `[RESOLVED]` | HIGH | Anti-copy jitter implemented: `_apply_jitter()` adds random_uniform(-30,+30)s time and random_choice([-1,0,+1]) size jitter to published signals; internal signals unchanged |
...
| G-ONL-032 | B7 | b7_position_monitor.py:134 | CLAUDE.md TZ rule | `[RESOLVED]` | HIGH | Time-exit and `_parse_close_time` now use `datetime.now(ZoneInfo("America/New_York"))` for timezone-aware comparison |
...
| G-ONL-036 | B7-Shadow | b7_shadow_monitor.py:165-170 | Reliability | `[RESOLVED]` | HIGH | Shadow monitor now has 3-attempt exponential backoff (0.5s, 1s, 2s) matching real B7 retry pattern |
...
| G-ONL-048 | Main | main.py:107-110 | Doc 33 crash recovery | `[RESOLVED]` | HIGH | Checkpoint branching implemented: detects mid-session crash (next_action not shutdown/initialization), writes CRASH_RECOVERY checkpoint, logs recovery state |

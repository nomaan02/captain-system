...
const SignalExecutionBar = ({ className = "" }) => {
  ...
  return (
    <div
      className={`w-full border-[#1e293b] border-solid border-b box-border flex items-end justify-between pt-1.5 px-3 pb-[5px] gap-5 max-w-full text-left text-[15px] text-[#64748b] font-['JetBrains_Mono'] md:flex-wrap md:gap-5 ${className}`}
    >
      {/* Title + pipeline pills */}
      <div className="w-full flex items-start gap-3 max-w-xl md:flex-wrap">
        ...
        <div data-testid="session-phase" className="flex-1 flex items-start gap-1.5 min-w-[231px] max-w-full text-xs sm:flex-wrap">
          {STAGES.map((stage) => {
